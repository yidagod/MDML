 ...
 "cells": [
  {
   ...
   "id": "96b7629a-c903-4a34-adc3-8890511ce9c1",
   "metadata": {},
   "outputs": [],
   "source": [
    "# 导入必要的库\n",
    ...
    "\n",
    "# 使用 gzip 加载压缩的 DML 模型\n",
    "with gzip.open('dml_model.pkl.gz', 'rb') as f:\n",
    "    dml = joblib.load(f)\n",
    "\n",
    "# 加载数据（用于获取特征名称和范围）\n",
    ...
    "            input_features[col] = st.sidebar.date_input(col, value=data[col].mean().floor('D'))\n",
    "        else:\n",
    "            st.sidebar.write(f\"Unsupported data type for column {col}: {dtype}\")\n",
    "    return input_features\n",
    "\n",
    "# 主应用\n",
    "def main():\n",
    "    st.title('中老年人心血管疾病风险评估系统')\n",
    "    \n",
    "    # 输入特征\n",
    ...
    "        if 'exercise' in input_df.columns:\n",
    ...
    "            st.plotly_chart(fig)\n",
    "        else:\n",
    "            st.write(\"警告：数据中未找到 'exercise' 列，无法计算因果效应。\")\n",
    "\n",
